...
def overlay_head_video(img_dir: Path, df: pd.DataFrame, head_points: np.ndarray, 
    ...
    try:
        if prepare_overlay_df is not None and df is not None:
            # use a small threshold (0.0) to treat exact zeros as missing
    ...
    if len(df) > 0:
        ...
        for i in range(len(df)):
            ...
            if is_valid_overlay_point is not None:
                valid = is_valid_overlay_point(hx0, hy0, w, h)
            else:
                valid = not (np.isnan(hx0) or np.isnan(hy0)) and not (hx0 == 0.0 and hy0 == 0.0)
            if valid:
    ...
    if len(df) > 0:
        for i in range(len(df)):
            ...
            if not (np.isnan(lax) or np.isnan(rax)):
                ...
                # validate mid point against image
                try:
                    if 'is_valid_overlay_point' in locals() and is_valid_overlay_point is not None:
                        if mid_x_scaled is not None and not (np.isnan(mid_x_scaled)) and is_valid_overlay_point(mid_x_scaled, y_ref, w, h):
    ...
    for i, p in enumerate(images):
        ...
        valid_nose = False
        try:
            if 'is_valid_overlay_point' not in locals() or is_valid_overlay_point is None:
                try:
                    from .runner_utils import is_valid_overlay_point
                except Exception:
                    try:
                        ...
                        is_valid_overlay_point = None
            if is_valid_overlay_point is not None:
                valid_nose = is_valid_overlay_point(hx, hy, w, h)
            else:
                valid_nose = not (np.isnan(hx) or np.isnan(hy)) and not (hx == 0.0 and hy == 0.0)
        except Exception:
            valid_nose = not (np.isnan(hx) or np.isnan(hy)) and not (hx == 0.0 and hy == 0.0)
