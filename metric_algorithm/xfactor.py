...
def compute_xfactor(df: pd.DataFrame) -> Dict[str, any]:
    ...
    # sanitize numeric outputs to avoid NaN/Inf being emitted into JSON
    result = {
        'chosen_plane': best_plane,
        'xfactor_max_deg': _safe_num_to_python(stats[best_plane]['xfactor_max_deg']),
        'xfactor_max_frame': int(stats[best_plane].get('xfactor_max_frame', 0) or 0),
        'xfactor_at_impact_deg': _safe_num_to_python(stats[best_plane]['xfactor_at_impact_deg']),
...
def overlay_xfactor_video(
    ...
    try:
        if prepare_overlay_df is not None and df_overlay is not None:
            df_overlay = prepare_overlay_df(df_overlay, prefer_2d=True, zero_threshold=0.0)
    except Exception:
        pass
